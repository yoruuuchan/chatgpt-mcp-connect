...
| **Cloudflare Worker** | a small Worker | Workers KV | You want auth to stay up when the workstation sleeps ([comfyui](../recipes/comfyui/)) |
...
In every case the origin is `http://127.0.0.1:<port>` — the local port stays bound to loopback and is never exposed directly.

### 4. Supervision: what keeps it running?

A connector that works until the next reboot is not finished. Each recipe has three or four processes that must all be alive: the MCP server, sometimes a bridge, sometimes a gateway, and the tunnel.

The pattern used throughout this repo is a single supervisor script started by a scheduled task at logon, which polls every few seconds and restarts whatever died, guarded by a named mutex so duplicate supervisors can't stack. See [`templates/supervisor`](../templates/supervisor/).
