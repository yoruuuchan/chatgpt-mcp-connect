...
**0. Decide whether you are exposing one MCP or a runtime.** If you want one persistent ChatGPT connector in front of multiple Workspaces, Skills, or upstream MCP servers, start with [`recipes/mcpx`](./recipes/mcpx/). MCPX is the aggregation/runtime path; the rows below are the direct leaf-server paths.

**1. Find your row.** Answer two questions about the MCP server you want to connect directly:
...
For a server with built-in OAuth such as MCPX, omit `--gateway` and point `--upstream` at the server itself.
...
**This gives you** a working, authenticated, public MCP endpoint that survives a reboot, and a way to tell which layer is broken when it isn't working. It also documents when a single runtime connector is a better fit than exposing every leaf MCP independently.
...
| [opentokenz/mcpx](https://github.com/opentokenz/mcpx) | Apache-2.0 | MCP runtime / aggregation recipe |
| [sugarforever/amap-mcp-server](https://github.com/sugarforever/amap-mcp-server) | see upstream | upstream-MCP example inside the MCPX recipe |
